...
public:
  ...
  // Getters for Slope Detection
  float GetSurfaceSlope() const {
    return _surfaceSlope.load(std::memory_order_relaxed);
  }
  float GetMacroFrontSlope() const {
    return _macroFrontSlope.load(std::memory_order_relaxed);
  }
  float GetMacroLeftSlope() const {
    return _macroLeftSlope.load(std::memory_order_relaxed);
  }
  float GetMacroRightSlope() const {
    return _macroRightSlope.load(std::memory_order_relaxed);
  }

private:
  static constexpr float CAP_HEIGHT = 4000.0f;

  void UpdateObstacleDetection(RE::PlayerCharacter *a_player,
  ...
  float UpdateVerticality(RE::TESGlobal *a_global,
                          ...
                          VerticalityHitInfo *a_outHitInfo = nullptr,
                          bool a_skipPreCheck = false,
                          bool a_allowNegativeDiff = false);
  ...
  // Slope Globals
  RE::TESGlobal *_surfaceSlopeGlobal{nullptr};
  RE::TESGlobal *_macroFrontSlopeGlobal{nullptr};
  RE::TESGlobal *_macroLeftSlopeGlobal{nullptr};
  RE::TESGlobal *_macroRightSlopeGlobal{nullptr};

  RE::NiPoint3 _lastUpdatePos;
  float _lastUpdateAngle{0.0f};
  bool _initialized{false};
  bool _wasInFurniture{false};
  float _furnitureExitCooldown{0.0f};
  ...
  // Slope cache (Thread-safe for OAR)
  std::atomic<float> _surfaceSlope{0.0f};
  std::atomic<float> _macroFrontSlope{0.0f};
  std::atomic<float> _macroLeftSlope{0.0f};
  std::atomic<float> _macroRightSlope{0.0f};

  bool _wasInMidair{false};

  // Jump bonus state (fJumpHeightMin dynamic modification)
  bool _jumpBoosted{false};
  float _origJumpHeight{76.0f};
